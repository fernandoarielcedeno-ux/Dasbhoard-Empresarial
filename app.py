...
st.set_page_config(page_title='DASBHOARD EMPRESARIAL',
                   layout='wide')
...
def generar_pdf(df, df2, fig1, fig2, fig3):
    ...
    pio.write_image(fig1, tmp1.name, format='png')
    pio.write_image(fig2, tmp2.name, format='png')
    pio.write_image(fig3, tmp3.name, format='png')
